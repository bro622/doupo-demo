...
def check_static_res_refs(errors: list[str]) -> None:
    ...
    for path in PROJECT.rglob("*"):
        ...
        for match in pattern.finditer(text):
            ref = match.group(0)
            if "%" in ref or "{" in ref or ref.endswith("/%s.png"):
                continue
...
def check_export_config(errors: list[str]) -> None:
    ...
    if 'exclude_filter="addons/godot_mcp/*"' not in text:
        errors.append("export_presets.cfg must exclude development addon addons/godot_mcp/*")
